                    ...
                    for (var k in suffix_value_list) {				    //for(var w=0; w<hash_suffix.length; w++){			//foreach var $suffix_value (@{$$hash{"suf#$suffix"}}) {
			    		...
			    		if( table.table_BC[cat_b + "!" + cat_c]  ){
							...
							if(  table.table_AC[cat_a + "!" + cat_c] ){										       //if ( exists($$hash{"tAC#$cat_a $cat_c"}) ) {
                                ...
                                bw          = replaceAll('undefined','', bw);   //Replace undefined with empty
                                bw          = replaceAll("^\\++", '', bw)       //Replace pluses in the beginning with empty
                                bw          = replaceAll("\\++$", '', bw)       //Replace pluses in the ending with empty
                                bw          = replaceAll('\\++', "+", bw);      //Replace double pluses with single pluses
                                analysis.bw = bw
                                //console.log("bw:",bw)
...
var segmentword = function(str){
    ...
    while ( prefix_len <= max_prefix_len ) {
    	...
    	if ( hash_prefix.hasOwnProperty(prefix)) {                          // if (exists(hash["pre#$prefix"])) {
    	    ...
    	    while ((stem_len >= 1) && (suffix_len <= max_suffix_len)) {
                ...
                if (hash_suffix.hasOwnProperty(suffix)) {                      //	if (exists(hash["suf#$suffix"])) {
